...
locals {
  ...
  backend_custom_domain  = "devbackend.egzaminmaklerski.online"
  frontend_custom_domain = "devfrontend.egzaminmaklerski.online"
  grafana_custom_domain  = "devmonitoring.egzaminmaklerski.online"
...
resource "azurerm_storage_share_file" "prometheus_config_yml" {
  ...
  content_md5       = local_file.prometheus_scrape_config.content_md5
...
module "backend" {
  ...
  custom_domain_names          = [local.backend_custom_domain]
  secrets = [
...
module "frontend" {
  ...
  target_port                  = 80
  external_ingress             = true
  custom_domain_names = [local.frontend_custom_domain]
  env_vars = [
...
module "grafana" {
  ...
  target_port                  = 3000
  external_ingress             = true
  custom_domain_names          = [local.grafana_custom_domain]
  secrets = [
